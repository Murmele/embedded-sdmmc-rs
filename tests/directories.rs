...
fn fat16_root_directory_listing() {
    ...
    tokio_test::block_on(volume_mgr.iterate_dir(root_dir, |d| {
...
fn fat16_sub_directory_listing() {
    ...
    tokio_test::block_on(volume_mgr.iterate_dir(test_dir, |d| {
        if count == 0 {
...
fn delete_file() {
    ...
    tokio_test::block_on(volume_mgr.delete_file_in_dir(root_dir, "README.TXT")).unwrap();
...
fn make_directory() {
    ...
    tokio_test::block_on(volume_mgr.make_dir_in_dir(root_dir, &test_dir_name)).unwrap();

    let new_dir = tokio_test::block_on(volume_mgr.open_dir(root_dir, &test_dir_name)).unwrap();

    let mut has_this = false;
    let mut has_parent = false;
    tokio_test::block_on(volume_mgr.iterate_dir(new_dir, |item| {
    ...
    let new_file = tokio_test::block_on(volume_mgr.open_file_in_dir(
        new_dir,
        &test_file_name,
        embedded_sdmmc::Mode::ReadWriteCreate,
    ))
    .expect("open new file");
    tokio_test::block_on(volume_mgr.write(new_file, b"Hello")).expect("write to new file");
    ...
    assert!(tokio_test::block_on(volume_mgr.make_dir_in_dir(root_dir, &test_dir_name)).is_err());
    let new_dir =
        tokio_test::block_on(volume_mgr.open_dir(root_dir, &test_dir_name)).expect("find new dir");
    let new_file = tokio_test::block_on(volume_mgr.open_file_in_dir(
        new_dir,
        &test_file_name,
