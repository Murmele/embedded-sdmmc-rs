...
fn open_files() {
    ...
    // Open with string
    let f = tokio_test::block_on(volume_mgr
    ...
    assert!(matches!(
        tokio_test::block_on(volume_mgr.open_file_in_dir(root_dir, &dir_entry.name, Mode::ReadOnly)),
        Err(Error::FileAlreadyOpen)
    ));
    ...
    let f2 = tokio_test::block_on(volume_mgr
    ...
    // Create a new file
    let f3 = tokio_test::block_on(volume_mgr
...
fn open_non_raw() {
    ...
    let mut volume_mgr: VolumeManager<utils::RamDisk<Vec<u8>>, utils::TestTimeSource, 4, 2, 1> =
        VolumeManager::new_with_limits(disk, time_source, 0xAA00_0000);
    let mut volume = tokio_test::block_on(volume_mgr.open_volume(VolumeIdx(0))).expect("open volume");
    let mut root_dir = volume.open_root_dir().expect("open root dir");
    let mut f = tokio_test::block_on(root_dir
