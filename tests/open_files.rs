...
fn open_files() {
    ...
    let volume =
        tokio_test::block_on(volume_mgr.open_raw_volume(VolumeIdx(0), VolumeOpenMode::ReadWrite)).expect("open volume");
    ...
    tokio_test::block_on(volume_mgr.close_file(f)).expect("close file");

    // Open with SFN

    let dir_entry = tokio_test::block_on(volume_mgr.find_directory_entry(root_dir, "README.TXT"))
        .expect("find file");
    ...
    assert!(matches!(
        tokio_test::block_on(volume_mgr.open_file_in_dir(
            root_dir,
            &dir_entry.name,
            Mode::ReadOnly
        )),
    ...
    let f2 = tokio_test::block_on(volume_mgr.open_file_in_dir(
        root_dir,
        "64MB.DAT",
        Mode::ReadWriteTruncate,
    ))
    ...
    assert!(matches!(
        tokio_test::block_on(volume_mgr.open_file_in_dir(root_dir, "EMPTY.DAT", Mode::ReadOnly)),
        Err(Error::TooManyOpenFiles)
    ));

    tokio_test::block_on(volume_mgr.close_file(f)).expect("close file");
    tokio_test::block_on(volume_mgr.close_file(f2)).expect("close file");

    // File not found

    assert!(matches!(
        tokio_test::block_on(volume_mgr.open_file_in_dir(root_dir, "README.TXS", Mode::ReadOnly)),
        Err(Error::NotFound)
    ));

    // Create a new file
    let f3 = tokio_test::block_on(volume_mgr.open_file_in_dir(
        root_dir,
        "NEWFILE.DAT",
    ...
    tokio_test::block_on(volume_mgr.write(f3, b"12345")).expect("write to file");
    tokio_test::block_on(volume_mgr.write(f3, b"67890")).expect("write to file");
    tokio_test::block_on(volume_mgr.close_file(f3)).expect("close file");

    // Open our new file
    let f3 =
        tokio_test::block_on(volume_mgr.open_file_in_dir(root_dir, "NEWFILE.DAT", Mode::ReadOnly))
            .expect("open file");
...
fn open_non_raw() {
    ...
    let mut volume_mgr: VolumeManager<utils::RamDisk<Vec<u8>>, utils::TestTimeSource, 4, 2, 1> =
        VolumeManager::new_with_limits(disk, time_source, 0xAA00_0000);
    let mut volume =
        tokio_test::block_on(volume_mgr.open_volume(VolumeIdx(0))).expect("open volume");
    let mut root_dir = volume.open_root_dir().expect("open root dir");
    let mut f = tokio_test::block_on(root_dir.open_file_in_dir("README.TXT", Mode::ReadOnly))
        .expect("open file");

    let mut buffer = [0u8; 512];
    let len = tokio_test::block_on(f.read(&mut buffer)).expect("read from file");
    ...
    let Err(Error::FileAlreadyExists) =
        tokio_test::block_on(root_dir.open_file_in_dir("README.TXT", Mode::ReadWriteCreate))
    else {
