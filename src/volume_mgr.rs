    ...
    <D as BlockDevice>::Error: core::fmt::Debug,
{
    ...
    pub async fn open_volume(
        ...
        return self._open_volume(volume_idx, VolumeOpenMode::ReadWrite).await;
    }
    ...
    pub async fn open_volume_read_only(
        ...
        return self._open_volume(volume_idx, VolumeOpenMode::ReadOnly).await;
    }
    /// Get a volume (or partition) based on entries in the Master Boot Record.
    ///
    /// We do not support GUID Partition Table disks. Nor do we support any
    ...
    async fn _open_volume(
        ...
        let v = self.open_raw_volume(volume_idx, open_mode).await?;
        if open_mode != VolumeOpenMode::ReadOnly {
            self.set_volume_status_dirty(v, true).await?;
        }
        Ok(v.to_volume(self))
    }
    ...
    pub async fn close_volume(&mut self, volume: RawVolume) -> Result<(), Error<D::Error>> {
        ...
        if self.open_volumes[volume_idx].open_mode != VolumeOpenMode::ReadOnly {
            self.set_volume_status_dirty(volume, false).await?;
        }
    ...
    pub async fn volume_status_dirty(&self, volume: RawVolume) -> Result<bool, Error<D::Error>> {
        let mut blocks = [Block::new()];
        let volume_idx = self.get_volume_by_id(volume)?;
...
mod tests {
    ...
    const FAT32_PARTITION0_BOOT: Block = Block {
        contents: [
    ...
    // same as FAT32_PARTITION0_BOOT with the following changes:
    // - fat table size reduced to 5 blocks (0x24..0x27)
    const FAT32_PARTITION0_BOOT_FAT_TABLE_SIZE_5: Block = Block {
        contents: [
            0xeb, 0x58, 0x90, 0x6d, 0x6b, 0x66, 0x73, 0x2e, 0x66, 0x61, 0x74, 0x00, 0x02, 0x08,
            0x02, 0x00, // 0x000
            0x02, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x00, 0x00, 0x10, 0x00, 0x04, 0x00, 0x00, 0x08,
            0x00, 0x00, // 0x010
            0x00, 0x20, 0x76, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00,
            0x00, 0x00, // 0x020
            0x01, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, // 0x030
            0x80, 0x01, 0x29, 0x0b, 0xa8, 0x89, 0x27, 0x50, 0x69, 0x63, 0x74, 0x75, 0x72, 0x65,
    ...
    const FAT32_PARTITION0_FSINFO: Block = // Partition 0 info sector (BPB_FSInfo)
        Block {
            contents: hex!(
    ...
    // FAT table with volume status NOT dirty
    const FAT32_PARTITION0_FAT_TABLE: Block = Block {
        contents: [
            0xF0, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, // 0x000
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, // 0x010
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    // FAT table with volume status dirty
    const FAT32_PARTITION0_FAT_TABLE_DIRTY: Block = Block {
        contents: [
            0xF0, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, // 0x000
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, // 0x010
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    impl DummyBlockDevice {
        // Actual blocks taken from an SD card, except
        // - changed the start and length of partition 0.
        // - FAT Table properties changed
        fn new_not_dirty() -> Self {
            Self {
                blocks: RefCell::new([
                    MBR_BLOCK,
                    FAT32_PARTITION0_BOOT,
                    FAT32_PARTITION0_FSINFO,
        ...
        // Size of the fat table set to 5 so both tables are accessible
        fn new_not_dirty_fattable_size_5() -> Self {
            Self {
                blocks: RefCell::new([
                    MBR_BLOCK,
    ...
    impl BlockDevice for DummyBlockDevice {
        ...
        async fn read(
            ...
            for (idx, block) in blocks.iter_mut().enumerate() {
                ...
                if block_idx < self.blocks.borrow().len() {
                    *block = self.blocks.borrow()[block_idx].clone();
                } else {
        ...
        async fn write(&self, _blocks: &[Block], _start_block_idx: BlockIdx) -> Result<(), Self::Error> {
            for idx in 0.._blocks.len() {
                self.blocks.borrow_mut()[_start_block_idx.0 as usize + idx] = _blocks[idx].clone();
            }
            Ok(())
        }
    ...
    fn partition0_dirty() {
        ...
        let v = tokio_test::block_on(c.open_raw_volume(VolumeIdx(0), VolumeOpenMode::ReadWrite)).unwrap();
        let expected_id = RawVolume(SearchId(0xAA00_0000));
        assert_eq!(v, expected_id);
        assert_eq!(
            &c.open_volumes[0],
    ...
    fn partition0_set_dirty() {
        ...
        assert_eq!(
            ...
            &VolumeInfo {
                ...
                volume_type: VolumeType::Fat(crate::FatVolume {
                    ...
                    num_blocks: BlockCount(0x0011_2233),
                    blocks_per_cluster: 8,
                    first_data_block: BlockCount(12),
                    fat_start: BlockCount(2),
                    fat_size: BlockCount(5),
                    fat_nums: 2,
                    name: fat::VolumeName::new(*b"Pictures   "),
                    free_clusters_count: None,
                    next_free_cluster: None,
                    cluster_count: 967_678,
                    fat_specific_info: fat::FatSpecificInfo::Fat32(fat::Fat32Info {
                        first_root_dir_cluster: ClusterId(2),
                        info_location: BlockIdx(1) + BlockCount(1),
                    })
        ...
        assert_eq!(c.block_device.blocks.borrow()[5], DUMMY);
        assert_eq!(c.block_device.blocks.borrow()[6], DUMMY);
